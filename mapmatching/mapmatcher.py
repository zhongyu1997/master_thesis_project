...
try:
    ...
    print("Error: missing the libraries arcpy")
    sys.exit()
...
try:
    ...
    print("Error: missing the libraries networkx")
    sys.exit()

gp = arcgisscripting.create()

def mapMatch(track, segments, decayconstantNet = 100, decayConstantEu = 100, maxDist = '0.03 kilometers', addfullpath = True, multipath = True):
    """
    ...
    points, points_info = getTrackPoints(track, segments)
    ...
    # read igs file
    # timestamp = SatellitePosition.uk_time_to_gps_week('20210325155710') # 0325
    # SatellitePosition.read_igs_files(timestamp, timestamp)

    # timestamp = SatellitePosition.uk_time_to_gps_week('20210401155710')  # 0401
    # SatellitePosition.read_igs_files(timestamp, timestamp)

    # timestamp = SatellitePosition.uk_time_to_gps_week('20210405155710')  # 0405
    # SatellitePosition.read_igs_files(timestamp, timestamp)

    timestamp = SatellitePosition.uk_time_to_gps_week('20210407155710')  # 0407
    SatellitePosition.read_igs_files(timestamp, timestamp)

    #init first point
    sc = getSegmentCandidates(points[0], points_info[0], segments, decayConstantEu, maxDist, multipath)
    for s in sc:
    ...
    for t in range(1, len(points)):
        ...
        sc = getSegmentCandidates(points[t], points_info[t], segments, decayConstantEu, maxDist, multipath)
        for s in sc:
            max_tr_prob = 0
            prev_ss = None
            path = []
            ...
            # gp.AddMessage("max_prob: %s " % V[t][s]["prob"])
        maxv = 1
        try:
            ...
            gp.AddMessage("Empty sequence again. Deleting current observation point...")
...
def getpointMatches(points, path):
    qr =  '"id" IN ' +str(tuple(path))
    arcpy.SelectLayerByAttribute_management('segments_lyr',"NEW_SELECTION", qr)
    opta = []
    for point in points:
        sdist = 100000
        candidate = ''
        cursor = arcpy.da.SearchCursor('segments_lyr', ["id", "SHAPE@"])
        for row in cursor:
...
def exportPath(opt, trackname):
    ...
    qr =  '"id" IN ' +str(tuple(map(lambda x: int(x), opt)))
    print qr
    # outname = (os.path.splitext(os.path.basename(trackname))[0][:9])+'_pth'
    outname = DataPath.intermedia_output_path + '\\' + trackname
    arcpy.SelectLayerByAttribute_management('segments_lyr',"NEW_SELECTION", qr)
    try:
        if arcpy.Exists(outname):
            arcpy.Delete_management(outname)
        arcpy.FeatureClassToFeatureClass_conversion('segments_lyr', DataPath.intermedia_output_path, trackname)
...
def getSegmentCandidatesMulti(point, point_info, decayConstantEu, maxdist='0.03 Kilometers'):
    print point_info[0]
    candidates = {}
    measure_GPS = ReadDEMDTM.get_DSM_height(point_info[4])
    measure_GPS = AbusolutePositioning.longlat_to_earthfixed(measure_GPS)

    ############### 2. get satellites position using the timestamp ###################
    tmp_time = SatellitePosition.uk_time_to_gps_week(str(point_info[2]))
    satellites_loc = SatellitePosition.lagrangian_intepolation(tmp_time, 9)
    avail_stl = point_info[3].split(',')
    if len(avail_stl) >= 2:
        ...
        seg_cursor = arcpy.da.SearchCursor('segments_lyr', ["id", "SHAPE@"])
        segs = []
        for seg in seg_cursor:
            segs.append(seg)
        del seg_cursor

        # if there is only one candidate, its possibility should be 1
        if len(segs) < 2:
            # return candidates
            return {segs[0][0]:1}
        ...
        for seg in segs:
            ...
            # arcpy.CopyFeatures_management(point, tmp_row)
            ...
            candidates_info = GeometricTools.get_points_along_polyline(point_info[4], seg[1])
            for can_info in candidates_info:
                # get candidate height
                candidate = (can_info[0], can_info[1])
                ...
                for sate_ind in avail_stl:
                    ...
                    multi_error = GeometricTools.multipath_error_calc(satellites_loc[sate_ind - 1], candidate,
                                                                      buildings_info)
                ...
                meter2lonlat = 0.1 # approximate ratio
                # dist = point.distanceTo(seg[1]) # degree
                dist = GeometricTools.distance_between(measure_GPS,
                                                       AbusolutePositioning.longlat_to_earthfixed((can_info[0], can_info[1], height)))

                if max(pseudo_errors) == -1:
                    candidates[seg[0]] = getNDProbability(dist*1000,decayConstantEu)  ##### ?????
                    continue
                ...
                dist2 = GeometricTools.distance_between(hypothesis_GPS, measure_GPS)
                print 'distance between hypothesis GPS point and measured GPS point is (km):', dist2
                # candidates[seg[0]] = dist2 + getNDProbability(dist,decayConstantEu)
                candidates[seg[0]] = 0.5*getNDProbability(dist2*1000,decayConstantEu)+0.5*getNDProbability(dist,decayConstantEu)

    return candidates

def getSegmentCandidates(point, point_info, segments, decayConstantEu, maxdist, multipath = True):
    """
    Returns closest segment candidates with a-priori probabilities.
    Based on maximal spatial distance of segments from point.
    """
    global gp
    if multipath and point_info[1] and point_info[1] < 8:
        tmp_candidate =  getSegmentCandidatesMulti(point,point_info,decayConstantEu,maxdist)
    ...
    arcpy.SelectLayerByLocation_management ("segments_lyr", "WITHIN_A_DISTANCE", point, maxdist) # segments will be selected even if most part of the segments are far away from our point
    candidates = {}
    ...
    for row in cursor:
        # feat = row[1]
        candidates_info = GeometricTools.get_points_along_polyline(point_info[4], row[1])[0]
        measure_GPS = ReadDEMDTM.get_DTM_height(point_info[4])
        candidates_ef = AbusolutePositioning.longlat_to_earthfixed((candidates_info[0],
                                                                     candidates_info[1],
                                                                    measure_GPS[2]))
        ef_GPS = AbusolutePositioning.longlat_to_earthfixed(measure_GPS)
        dist = GeometricTools.distance_between(ef_GPS,candidates_ef)
        #compute the spatial distance
        # dist = point.distanceTo(row[1])

        #compute the corresponding probability
        candidates[row[0]] = getPDProbability(dist*1000, decayConstantEu)
    del row
    del cursor
    #print str(candidates)
    # gp.AddMessage("the output is: %s " % candidates)
    return candidates
...
def getTrackPoints(track, segments):
    ...
    if arcpy.Exists(track):
        for row in arcpy.da.SearchCursor(track, ["SHAPE@",'Name', 'Signal_Num', 'Time', 'Avail_Stl','SHAPE@XY']):
            ...
            points_info.append(row[1:])
        print('track size:' + str(len(trackpoints)))
        return trackpoints,points_info
    else:
...
def getSegmentInfo(segments):
    ...
    if arcpy.Exists(segments):
        cursor = arcpy.da.SearchCursor(segments, ["id", "SHAPE@"])
        endpoints = {}
        segmentlengths = {}
        for row in cursor:
